...
that role can only touch this repo's own `neobank-<env>-neo-05` stack. You never hold AWS
credentials yourself — everything that reaches AWS goes through a workflow in this repo.

The front-end image is built with `APP_BASE_PATH=/neo-05`, because in the deployed stack
...
|---|---|---|---|
| **dev** | [`/neo-05/`](http://neobank-dev-571740187.ap-southeast-1.elb.amazonaws.com/neo-05/) | [`/neo-05/health`](http://neobank-dev-571740187.ap-southeast-1.elb.amazonaws.com/neo-05/health) · [`/neo-05/info`](http://neobank-dev-571740187.ap-southeast-1.elb.amazonaws.com/neo-05/info) | [orchestrator](http://neobank-dev-571740187.ap-southeast-1.elb.amazonaws.com/) |
| **prod** | [`/neo-05/`](http://neobank-prod-294820685.ap-southeast-1.elb.amazonaws.com/neo-05/) | [`/neo-05/health`](http://neobank-prod-294820685.ap-southeast-1.elb.amazonaws.com/neo-05/health) · [`/neo-05/info`](http://neobank-prod-294820685.ap-southeast-1.elb.amazonaws.com/neo-05/info) | [orchestrator](http://neobank-prod-294820685.ap-southeast-1.elb.amazonaws.com/) |
...
A `reset` destroys **only this service's schema** (`neo_05`). Every other service and the
...
DB_URL=jdbc:mysql://localhost:3307/neo_05 ./mvnw spring-boot:run
```
...
```json
{ "serviceId": "neo05", "status": "ACCEPTED",
...
| Env | Default | What |
|---|---|---|
| `SERVICE_ID` | `neo05` | the id sent on callbacks — note **no `-a`**, unlike the repo name |
| `SERVICE_NAME` | `Credit Decisioning` | display name |
...
- `backend/` and `frontend/` **stay at the repo root** — the system compose builds
  `./neo-05/backend` and `./neo-05/frontend` by path.
